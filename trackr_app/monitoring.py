...
from .models import Delivery, Invitation, NotionSync, WorkerState, utcnow
...
def operational_status(db):
    ...
    return {'status': 'degraded' if stale or errors or failures or delayed else 'ok', 'stale_workers': stale, 'workers_with_errors': errors, 'failed_tasks': failures, 'delayed_immediate_alerts': delayed}
