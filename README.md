...
Set `ADMIN_EMAIL` to bootstrap a new administrator, then request a link from `/login`. In production, SMTP is used by the scheduled GitHub email worker to deliver sign-in links; Vercel only records the request. Logs never contain these links. Gmail application-password separators are removed automatically. Other SMTP passwords are preserved.
...
For Vercel, use a pooled Neon URL as `DATABASE_URL` and, preferably, a direct Neon URL as `MIGRATION_DATABASE_URL`. If the latter is not configured, the production workflow uses `DATABASE_URL` for migration so workers are never stranded on an incompatible schema. Production and preview must each have their own database, `SECRET_KEY`, `ENCRYPTION_KEY` and `APP_URL`; a preview also requires `PREVIEW_ISOLATED=true`. SMTP credentials are configured for the production GitHub email worker. Keep the same SMTP secrets in Vercel as a fallback, but the Vercel runtime has no email-delivery path and never contacts SMTP. The GitHub deployment creates a candidate without assigning the public domain, verifies it, pauses and drains workers, migrates, promotes it, and verifies the rollback path if promotion fails.

Public link and password-recovery requests persist an encrypted recipient in the `auth_mail` queue without querying accounts. Vercel returns after that durable write and never calls SMTP or starts a background send. Every ten minutes, the GitHub `email-delivery` workflow drains auth requests first, then invitations, immediate alerts and daily digests. Each auth attempt persists a fresh 15-minute token before SMTP; failures retry with backoff, up to five attempts. GitHub scheduling is best effort, so transactional email can take more than ten minutes to arrive. Public requests more than one hour old are cancelled. Reset/deactivation/admin recovery cancel pending or processing auth emails as well as revoking issued credentials. Failed public auth jobs and queues delayed more than 30 minutes are reported by `/admin/operations`; worker output includes auth-mail statuses and errors. A crash after SMTP acceptance can still cause a duplicate email; its already persisted token remains valid unless revoked or expired.
...
Invitations persist before email delivery. The GitHub email workflow sends them only after they are queued, with a fresh 15-minute link on each retry; failures use exponential delay and a five-attempt limit. Their status is visible in `/admin`; inviting again retries a failed invitation. Repeated successful invitations within one minute are suppressed. Login responses remain generic.
...
`/health` checks database/schema and commit for deployment readiness. `/admin/operations` is administrator-only and reports stale workers (30 minutes), failed tasks, authentication emails delayed over 30 minutes, and immediate notifications delayed over one hour. `check-operations` exposes the same result to the scheduler and returns nonzero when degraded. Sources are monitored independently. Daily digests waiting for their scheduled hour are not flagged as delayed immediate mail.
...
Production uses Vercel, PostgreSQL, SMTP and GitHub Actions. Vercel records durable mail work only; GitHub Actions is the sole production SMTP sender for auth links, invitations, immediate alerts and daily digests. Keep `SECRET_KEY` and `ENCRYPTION_KEY` identical between web and workers. Set `ENVIRONMENT=production`, an HTTPS `APP_URL`, and the secrets listed in `.env.example`. The configuration synchronizer also propagates `NOTION_SYNC_ENABLED`.

The production workflow tests PostgreSQL, migrates and deploys, then checks the release. During deployment its database gate pauses new claims and drains the platform, email and historical workers so they do not run across a schema migration. It deliberately leaves the GitHub workflows enabled so scheduled-run notifications remain assigned to their human owner. The email workflow has separate timeouts and continues through its queued stages before reporting any aggregated failure. Collection completion triggers matching, and matching completion triggers email delivery; staggered schedules remain as recovery paths because GitHub schedules are not an exact-time delivery guarantee. Git auto-deploy remains disabled. Rollback retains additive migrations; rolling back across a schema revision requires verifying compatibility of the target release's health check and models.
