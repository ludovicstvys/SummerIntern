...
For Vercel, use a pooled Neon URL as `DATABASE_URL` and a direct Neon URL as `MIGRATION_DATABASE_URL`. Production and preview must each have their own database, `SECRET_KEY`, `ENCRYPTION_KEY`, `APP_URL` and SMTP credentials; a preview also requires `PREVIEW_ISOLATED=true`. The GitHub deployment creates a candidate without assigning the public domain, verifies it, pauses and drains workers, migrates, promotes it, and verifies the rollback path if promotion fails.
